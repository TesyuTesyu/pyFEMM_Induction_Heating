...
mesh_size_work=0.1#加熱対象の導体内部のメッシュサイズ.
...
Temp=600#温度. degree.
freq_min=1e2#周波数の下限.
freq_max=10e3#周波数の上限.
freq_num=2#周波数の分割数.

#結果ファイルの保存path.
file_path_csv_write=r"C:\Users\???\hgghfgh.csv"
...
femm.mi_addblocklabel(x_work,y_work)
...
femm.mi_selectlabel(x_work,y_work)
femm.mi_setblockprop('Work', 0, mesh_size_work, '<None>', 0, 0, 0)
femm.mi_clearselected()
...
for freq in matf:
    ...
    femm.mo_selectblock(x_work,y_work)
    P_work=femm.mo_blockintegral(4)
    P_work=np.real(P_work)
